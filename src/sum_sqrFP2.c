...
{
    /* 11.24.95 : JM Maisog : Calculates voxel-by-voxel sum of squared
                          voxel values of floating point images.        */
    ...
    if (argc < 3) {
        printf("sum_sqrFP2: need input file(s) and output file.\n\n");
        printf("This program will return a map of the voxel-by-voxel\n");
        printf("sum of squared voxel values of floating point images.\n");
        exit(1);
    }
    ...
    if ((fIN=fopen(argv[1],"r")) == NULL) {
        printf("sum_sqrFP2: can't open %s\n",argv[1]);
        exit(2);
    }
    ...
    fclose(fIN);
    num_vox = num_voxX4/4;
    printf("sum_sqrFP2: Number of voxels = %d\n",num_vox);
    ...
    if (imgin == NULL) {
        printf("sum_sqrFP2: failed to malloc for imgin.\n");
        exit(4);
    }

    sum = (float *) malloc(num_vox*sizeof(float));
    if (sum == NULL) {
        printf("sum_sqrFP2: failed to malloc for sum.\n");
        exit(5);
    }
    ...
    printf("sum_sqrFP2: opening file %s...\n",argv[argc-1]);
    if ((fOUT=fopen(argv[argc-1],"w")) == NULL) {
        printf("sum_sqrFP2: can't open %s\n",argv[argc-1]);
        exit(3);
    }
    printf("sum_sqrFP2: writing to %s...\n",argv[argc-1]);
