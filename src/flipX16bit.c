...
main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
	printf("flipX16bit: need rootname of input ANALYZE .img file\n");
	...
	printf("flipX16bit: can't open %s\n",headerfile);
	exit(1);
    }
    if(fread(&img_info, sizeof(struct analyze_struct), 1, fhdr) != 1){
	printf("flipX16bit: error reading %s\n",headerfile);
    ...
    if (imgin == NULL) {
	printf("flipX16bit: failed to malloc for imgin.\n");
	exit(4);
    }
     
    imgout = (signed short int *) malloc(num_vox*sizeof(signed short int));
    if (imgout == NULL) {
	printf("flipX16bit: failed to malloc for imgout.\n");
	exit(4);
    }
	...
	printf("flipX16bit: unable to open %s.\n",in_name);
	exit(9);
    }
    printf("flipX16bit: reading in %s...\n",in_name);
	...
	for (y=0; y<ydim; y++)
	    for (x=0; x<xdim; x++)
		imgout[(z*ydim+y)*xdim+x]=imgin[(z*ydim+y)*xdim+xdim-1-x];
