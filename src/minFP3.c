...
main (int argc, char *argv[])
{
    ...
    if ((argc!=2) && (argc!=3)) {
        printf("minFP3: need input floating point image file.\n");
        printf("         Will return the min of all voxels in this file.\n");
        printf("         If you include a second argument, name of an 8-bit\n");
        printf("         mask, only voxels in the mask will be checked.\n");
        exit(1);
    }
    ...
    if (imgin == NULL) {
        printf("minFP3: failed to malloc for imgin.\n");
        exit(4);
    }
    ...
    if (argc==3) {
        ...
        if ((fIN=fopen(argv[2],"r"))==NULL) {
            printf("minFP3: unable to open %s\n",argv[2]);
            exit(5);
        }
        else {
            ...
            if (read_status<num_vox) {
                printf("minFP3: only %d voxels read in from %s\n",read_status,argv[2]);
    ...
    for (vox_index=1; vox_index<num_vox; vox_index++)
        if(((argc==3)&&mask[vox_index]!=0) || (argc==2))
            min = (imgin[vox_index]<min) ? imgin[vox_index] : min;
