...
/* -------- */
{
  ...
  fprintf (stderr, "\n Where options are:");
  fprintf (stderr, "\n    -point #_of_point_diameter  -initial setting for how large the points in the time course should be");
...
/* ------------------ */
{
   ...
   register int       II, J, IJ, IJk, ijl, k0, IM2, lx2, ly, IJK;
      ...
      for (i=256*16; i < 256*32; i++)
       {  m = i%64 + 64 + 320*(63-(i-256*16)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*32; i < 256*48; i++)
       {  m = i%64 + 128 + 320*(63-(i-256*32)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*48; i < 256*64; i++)
       {  m = i%64 + 192 + 320*(63-(i-256*48)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*64; i < 256*80; i++)
       {  m = i%64 + 256 + 320*(63-(i-256*64)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*80; i < 256*96; i++)
       {  m = i%64 + 320*64 + 320*(63-(i-256*80)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*96; i < 256*112; i++)
       {  m = i%64 + 320*64 + 64 + 320*(63-(i-256*96)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*112; i < 256*128; i++)
       {  m = i%64 + 128 + 320*64 + 320*(63-(i-256*112)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*128; i < 256*144; i++)
       {  m = i%64 + 192 + 320*64 + 320*(63-(i-256*128)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*144; i < 256*160; i++)
       {  m = i%64 + 256 + 320*64 + 320*(63-(i-256*144)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*160; i < 256*176; i++)
       {  m = i%64 + 320*128 + 320*(63-(i-256*160)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*176; i < 256*192; i++)
       {  m = i%64 + 64 + 320*128 + 320*(63-(i-256*176)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*192; i < 256*208; i++)
       {  m = i%64 + 128 + 320*128 + 320*(63-(i-256*192)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*208; i < 256*224; i++)
       {  m = i%64 + 192 + 320*128 + 320*(63-(i-256*208)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*224; i < 256*240; i++)
       {  m = i%64 + 256 + 320*128 + 320*(63-(i-256*224)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*240; i < 256*256; i++)
       {  m = i%64 + 320*192 + 320*(63-(i-256*240)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*256; i< 256*272; i++)
       {  m = i%64 + +64 + 320*192 + 320*(63-(i-256*256)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*272; i< 256*288; i++)
       {  m = i%64 + 128 + 320*192 + 320*(63-(i-256*272)/64);
          tmp_imx[m] = imx[m] =  a[i]; }
  
      for (i=256*288; i< 256*304; i++)
       {  m = i%64 + 192 + 320*192 + 320*(63-(i-256*288)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*304; i< 256*320; i++)
       {  m = i%64 + 256 + 320*192 + 320*(63-(i-256*304)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*320; i< 256*336; i++)
       {  m = i%64 + 320*256 + 320*(63-(i-256*320)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*336; i< 256*352; i++)
       {  m = i%64 + 64 + 320*256 + 320*(63-(i-256*336)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*352; i< 256*368; i++)
       {  m = i%64 + 128 + 320*256 + 320*(63-(i-256*352)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*368; i< 256*384; i++)
       {  m = i%64 + 192 + 320*256 + 320*(63-(i-256*368)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*384; i< 256*400; i++)
       {  m = i%64 +  256 + 320*256 + 320*(63-(i-256*384)/64);
          tmp_imx[m] = imx[m] =  a[i]; }
