...
float threshold;        /* Lower threshold for probabilities.                   */
...
*/
{
    ...
    if ((argc != 8) && (argc!=9)) {
        printf("clusterGE_nocorners: Need name of input .hdr file, input image file,\n");
    ...
    if (image==NULL) {
        printf("clusterGE_nocorners: unable to malloc for image.\n");
        exit(2);
    }
    printf("Loading %s...\n",argv[2]);
    if ((fIN = fopen(argv[2],"r")) == NULL) {
        printf ("clusterGE_nocorners: can't open %s\n",argv[2]);
        exit(2);
    }
    num_read_in=fread(image,sizeof(float),num_vox,fIN);
    fclose(fIN);
    if(num_read_in!=num_vox) {
        printf("clusterGE_nocorners: %d voxels read in; should have been %d.\n",
                                                num_read_in,num_vox);
        exit(3);
    }
    ...
    if (clustermask==NULL) {
        printf("clusterGE_nocorners: unable to malloc for clustermask.\n");
        exit(4);
    }
    clustermask2 = (int *) malloc(sizeof(int)*num_vox);
    if (clustermask2==NULL) {
        printf("clusterGE_nocorners: unable to malloc for clustermask2.\n");
        exit(4);
    }
    outimg = (unsigned short *) malloc(sizeof(unsigned short)*num_vox);
    if (outimg==NULL) {
        printf("clusterGE_nocorners: unable to malloc for outimg.\n");
        exit(4);
    }
...
*/
    ...
    /* ****************************************************************** */
    /* COUNT NUMBER OF VOXELS GREATER THAN/EQUAL TO threshold AND != 0.0  */
    ...
    for (vox_index=0; vox_index<num_suprathresh_vox; vox_index++) {
        ...
        /* WHERE THE CORRESPONDING VOXEL IN image IS GREATER THAN         */
    ...
    if(num_wrote_out!=num_vox) {
        printf("clusterGE_nocorners: %d voxels wrote out; should have been %d.\n",
