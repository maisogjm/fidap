...
main(int argc, char *argv[])
{
    ...
    int duration;           /* Duration of an epoch in the input stimulus       */
                            /* waveform.                                        */
...
/*  CHECK TO SEE IF CORRECT NUMBER OF ARGUMENTS WERE PASSED.                    */
    ...
    if (argc<15) {
        printf("Dmultivar_multiple_regressionB: need input header file, input textfile\n");
...
/*  FOR ALL IMAGES.                                                             */
    ...
    zdimXnum_rows = (int)TR*tick*num_rows;
...
/* set slice_seq2[k-1] equal to (n-1).                                          */
    ...
    if ((ftxt = fopen(argv[8],"r")) == NULL) {
        printf("Dmultivar_multiple_regressionB: can't open %s\n",argv[8]);
        exit(2);
    }
    ...
    for (z=0; z<zdim; z++) {
        ...
        if (check_fscanf==EOF) {
            printf("Dmultivar_multiple_regressionB: error reading slice sequence from %s.\n",
                                                                        argv[8]);
            exit(3);
        }
    ...
    for (z=0; z<zdim; z++)
        if ((slice_seq1[z]<1) || (slice_seq1[z]>zdim)) {
            printf("Dmultivar_multiple_regressionB: weird values for slice sequence.\n");
...
/*  COUNT NUMBER OF INPUT IMAGE FILENAMES.                                      */
    ...
    if (tot_num_scans != num_dep_varXnum_rows) {
        printf("Dmultivar_multiple_regressionB: number of entries in %s not equal to number\n",argv[2]);
...
/*  ALLOCATE MEMORY FOR img_name; THEN READ IN NAMES OF INPUT IMAGE FILES.      */
    ...
    for (col_index=0; col_index<num_dep_var; col_index++)
        for (row_index=0; row_index<num_rows; row_index++) {
            ...
            if (check_fscanf==EOF) {
                printf("Dmultivar_multiple_regressionB: error reading run %d timepoint %d filename from %s.\n",
...
/* READ IN INDEPENDENT VARIABLES FROM TEXT FILES.                               */
    ...
    for (var_index=0; var_index<num_ind_var; var_index++) {
        ...
        t_pointer = 0;
        while (((check_fscanf=fscanf(f_indvar,"%d %f",&duration, &float_buffer))!=EOF)
            && (t_pointer+1<zdimXnum_rows)){
            for (t_index=t_pointer;(t_index<zdimXnum_rows) &&
                ((float)t_index<(float)t_pointer+((float)tick*duration)); t_index++) {
                X_os[t_index+1][var_index+1]=(double)float_buffer;
            }
            t_pointer=t_index;
        }
        fclose(f_indvar);
        if (t_index!=(int)TR*num_rows*tick) {
            printf("t_index = %d\n");
            printf("Dmultivar_multiple_regressionB: not enough data in %s to cover %g seconds.\n",
...
for(row_index=0;row_index<num_rows;row_index++) {
    for(var_index=0;var_index<num_ind_var;var_index++)
        fprintf(fOUTFILE,"%10f ",HDresponse[var_index][row_index]);
    fprintf(fOUTFILE,"\n");
}
fclose(fOUTFILE);
*/
...
/*            if (isinf(Wilks_Lambda[vox_index])) {
                ...
                if ((fOUTFILE = fopen("B_hat.dat","w")) == NULL) {
                    printf("Dmultivar_multiple_regressionB: can't open B_hat.dat.\n");
                    exit(12);
                }
