...
/* -------- */
{
  ...
  fprintf (stderr, "\n Where options are:");
  fprintf (stderr, "\n    -point #_of_point_diameter - initial setting for how large the points in the time course should be. [def = 1]");
...
/* ------------------ */
{
   ...
   register int       II, J, IJ, IJk, ijl, k0, IM2, lx2, ly, IJK;
      ...
      for (i=256*16; i < 256*32; i++)
       {  m = i%64 + 64 + 384*(63-(i-256*16)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*32; i < 256*48; i++)
       {  m = i%64 + 128 + 384*(63-(i-256*32)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*48; i < 256*64; i++)
       {  m = i%64 + 192 + 384*(63-(i-256*48)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*64; i < 256*80; i++)
       {  m = i%64 + 256 + 384*(63-(i-256*64)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*80; i < 256*96; i++)
       {  m = i%64 + 320 + 384*(63-(i-256*80)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*96; i < 256*112; i++)
       {  m = i%64 + 384*64 + 384*(63-(i-256*96)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*112; i < 256*128; i++)
       {  m = i%64 + 64 + 384*64 + 384*(63-(i-256*112)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*128; i < 256*144; i++)
       {  m = i%64 + 128 + 384*64 + 384*(63-(i-256*128)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*144; i < 256*160; i++)
       {  m = i%64 + 192 + 384*64 + 384*(63-(i-256*144)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*160; i < 256*176; i++)
       {  m = i%64 + 256 + 384*64 + 384*(63-(i-256*160)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*176; i < 256*192; i++)
       {  m = i%64 + 320 + 384*64 + 384*(63-(i-256*176)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*192; i < 256*208; i++)
       {  m = i%64 + 384*128 + 384*(63-(i-256*192)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*208; i < 256*224; i++)
       {  m = i%64 + 64 + 384*128 + 384*(63-(i-256*208)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*224; i < 256*240; i++)
       {  m = i%64 + 128 + 384*128 + 384*(63-(i-256*224)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*240; i < 256*256; i++)
       {  m = i%64 + 192 + 384*128 + 384*(63-(i-256*240)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*256; i< 256*272; i++)
       {  m = i%64 + 256 + 384*128 + 384*(63-(i-256*256)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*272; i< 256*288; i++)
       {  m = i%64 + 320 + 384*128 + 384*(63-(i-256*272)/64);
          tmp_imx[m] = imx[m] =  a[i]; }
  
      for (i=256*288; i< 256*304; i++)
       {  m = i%64 + 384*192 + 384*(63-(i-256*288)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*304; i< 256*320; i++)
       {  m = i%64 + 64 + 384*192 + 384*(63-(i-256*304)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*320; i< 256*336; i++)
       {  m = i%64 + 128 + 384*192 + 384*(63-(i-256*320)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*336; i< 256*352; i++)
       {  m = i%64 + 192 + 384*192 + 384*(63-(i-256*336)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*352; i< 256*368; i++)
       {  m = i%64 + 256 + 384*192 + 384*(63-(i-256*352)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*368; i< 256*384; i++)
       {  m = i%64 + 320 + 384*192 + 384*(63-(i-256*368)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*384; i< 256*400; i++)
       {  m = i%64 +  384*256 + 384*(63-(i-256*384)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*400; i< 256*416; i++)
       {  m = i%64 + 64 + 384*256 + 384*(63-(i-256*400)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*416; i< 256*432; i++)
       {  m = i%64 + 128 + 384*256 + 384*(63-(i-256*416)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*432; i< 256*448; i++)
       {  m = i%64 + 192 + 384*256 + 384*(63-(i-256*432)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*448; i< 256*464; i++)
       {  m = i%64 + 256 + 384*256 + 384*(63-(i-256*448)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*464; i< 256*480; i++)
       {  m = i%64 + 320 + 384*256 + 384*(63-(i-256*464)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*480; i< 256*496; i++)
       {  m = i%64 +  384*320 + 384*(63-(i-256*480)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*496; i< 256*512; i++)
       {  m = i%64 + 64 + 384*320 + 384*(63-(i-256*496)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*512; i< 256*528; i++)
       {  m = i%64 + 128 + 384*320 + 384*(63-(i-256*512)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*528; i< 256*544; i++)
       {  m = i%64 + 192 + 384*320 + 384*(63-(i-256*528)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*544; i< 256*560; i++)
       {  m = i%64 + 256 + 384*320 + 384*(63-(i-256*544)/64);
          tmp_imx[m] = imx[m] =  a[i]; }

      for (i=256*560; i< 256*576; i++)
       {  m = i%64 + 320 + 384*320 + 384*(63-(i-256*560)/64);
          tmp_imx[m] = imx[m] =  a[i]; }
...
/*   sprintf (strp,"Im. rot.");
   ...
   sprintf (strp,"voxel value: %d at vol. #%d", val[xc][yc][Im_Nr], Im_Nr+1);
   subW_TXT(str_x, 20, strp);
   loc = str_x;
