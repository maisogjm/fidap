...
main (int argc, char *argv[])
{
    ...
    if ((argc!=2) && (argc!=3)) {
        printf("meanFP2: need input floating point image file.\n");
    ...
    if (imgin == NULL) {
        printf("meanFP2: failed to malloc for imgin.\n");
        exit(4);
    }
    ...
    if (argc==3) {
        ...
        if ((fIN=fopen(argv[2],"r"))==NULL) {
            printf("meanFP2: unable to open %s\n",argv[2]);
            exit(5);
        }
        else {
            ...
            if (read_status<num_vox) {
                printf("meanFP2: only %d voxels read in from %s\n",read_status,argv[2]);
