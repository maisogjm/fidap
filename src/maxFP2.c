...
{
    /* 03.12.96 : JM Maisog : Finds max of each voxel across several    */
    ...
    if (argc < 3) {
        printf("maxFP2: need input file(s) and output file.\n");
        exit(1);
    }
    ...
    fclose(fIN);
    num_vox = num_voxX4/4;
    printf("maxFP2: Number of voxels = %d\n",num_vox);
    ...
    for (cmd_lin_arg = 1; cmd_lin_arg<=argc-2 ;cmd_lin_arg++) {
        ...
        for (vox_index=0; vox_index<num_vox; vox_index++)
            vox_max[vox_index]=(imgin[vox_index]>vox_max[vox_index]) 
